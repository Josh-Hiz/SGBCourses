Week 4: Objects and classes
===========================

.. toctree:: 
    :maxdepth: 1
    :caption: Week 4: Objects and classes

    slides/1_what_is_object
    slides/2_surprise
    slides/3_method_calls
    slides/4_creating_objects
    slides/5_using_constructs
    slides/6_defining_objects_class
    slides/7_constructors
    slides/8_working_fields
    slides/9_sum
    slides/10_defining_methods
    slides/11_adder
    slides/12_private_fields
    slides/13_SmallCounter
    slides/14_printing_objects
    slides/15_sum_with_repr
    slides/16_recap
    slides/17_finding_your_way
    slides/18_explaining
    slides/19_doctest
    slides/20_testing_mean
    slides/21_inheritance
    slides/22_inheritance
    slides/23_classy_test
    slides/24_testing_median