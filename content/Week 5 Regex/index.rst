Week 5: Regular expressions
===========================

.. toctree:: 
    :maxdepth: 1

    slides/1_strings_structured
    slides/2_DNA
    slides/3_DNA_encoding
    slides/4_translating_DNA
    slides/5_translating_DNA
    slides/6_motifs
    slides/7_finding_motifs
    slides/8_regular_expressions
    slides/9_character_classes
    slides/10_matching_practice
    slides/11_re_module
    slides/12_findall_practice
    slides/13_special_escape_raw
    slides/14_escaping_practice
    slides/15_parallel_composition
    slides/16_choice_practice
    slides/17_repetition
    slides/18_us_phone_nums
    slides/19_unbound_rep
    slides/20_search_match_fullmatch
    slides/21_practice_match
    slides/22_iso
    slides/23_subs
    slides/24_caution